function [alphaArray,offsetSize] = alpha030(dailyClose,MKT,SMB,HML)
...
for windowController = 1:m1-windowSize
    ...
    X4 = cat(4,tempX{:}); %create a 4-d matrix
    X3 = reshape(X4,[],3,60); %X3 of size #companies*#factors(=3)*#days
    X2 = mat2cell(X3, n1, 3, ones(1,60)); %each cell of size #companies*#factors
    X = blkdiag(X2{:}); % diagnalized
