function [] = fillDataUpdate(obj, tableNames)
% FILLDATA combines delete nan and fill nan
% fillMethod, 1x2 cell array, first element: method fillna in the
...
%       |'pchip'        | N/A(caution:future data!)
    ...
    % start feedStruct
    try
        feedStruct = obj.selectedStruct;
        fillMethod = obj.jsonDecoder(obj.defaultFillDataMethod);
    catch
        error 'feed struct error or fillMethod not a cell array';
    end
    
    % check validity of input: fieldName
    for fieldName = tableNames
    ...
    % 
    feedTables = cell(length(tableNames));
    reserveRecord = ones(1,size(feedStruct.(tableNames(1)),2));
    for count = 1:length(tableNames)
        fieldName = tableNames(count);
    ...
    feedTablesAfterDeletion = cell(length(tableNames));
    for count = 1:length(tableNames)
    ...
    for count = 1:length(tableNames)
        ...
        if sum(isnan(filledTable),'all') == 0
            disp([tableNames(count),': FILLING summary: no nan, finished!']);
        else
            ...
            disp([tableNames(count),': FILLING summary: nan exists!There are ',num2str(totalNans),' nans in all, '...
                ,num2str(size(totalCols,2)) ,' colums have nan.please use other pair of method or check mannually!']);
        end
        
        filledTables.(tableNames(count)) = filledTable;
